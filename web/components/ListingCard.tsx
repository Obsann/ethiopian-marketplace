...
export function ListingCard({
  ...
  return (
    <article className="group relative w-full min-w-0 max-w-full">
      <Link href={href || `/listings/${listing.id}`} className="block min-w-0 max-w-full cursor-pointer outline-none">
        <div
          className={`relative block w-full min-w-0 max-w-full overflow-hidden rounded-xl bg-stone-200 ${mediaAspectClass(listing, size)}`}
        >
          <SafeImage
            src={primary}
            alt={listing.title}
            ...
            className={`h-full w-full max-w-full object-cover ${objectPos} transition duration-700 ease-out group-hover:scale-[1.04] ${
          ...
          {secondary && (
            <SafeImage
              ...
              className={`h-full w-full max-w-full object-cover ${objectPos} opacity-0 transition duration-700 group-hover:opacity-100`}
              aria-hidden
            />
          )}
